...
(function (BABYLON) {
    var VertexData = (function () {
        ...
        VertexData.CreateGround = function (width, height, subdivisions) {
            console.log('myCreateGroundCalled');
            var indices = [];
            var positions = [];
            var normals = [];
            var uvs = [];
        ...
        VertexData.CreateGroundOffset = function (width, height, subdivisions, offsetx, offsetz) {
            console.log('createGroundOffset');
            var indices = [];
            var positions = [];
            var normals = [];
            var uvs = [];
            ...
            for (row = 0; row <= subdivisions; row++) {
                for (col = 0; col <= subdivisions; col++) {
                    var position = new BABYLON.Vector3(
                        (((col * width) / subdivisions - (width / 2.0)) + offsetx),
                        0, 
                        (((subdivisions - row) * height) / subdivisions - (height / 2.0)) + offsetz);
                    // console.log((subdivisions - row) * height);
                    // console.log(subdivisions - (height / 2.0));
                    // console.log(position);
                    var normal = new BABYLON.Vector3(0, 1.0, 0);
